"""Review page — visual grid for duplicate groups with glass morphism.

Workspace shell is assembled in ``pages/review/shell_attach.py``; behavior lives in
``pages/review/review_mixins.py`` and subviews (grid, inspector).
"""
...
class ReviewPage(
    ...
    """Grid view for visual triage of duplicate groups."""
    ...
    def load_group(self, groups: List[DuplicateGroup], group_id: int, mode: Optional[str] = None) -> None:
        ...
        if hasattr(page, "run_task"):
            page.run_task(self._finish_load_to_grid_async, group_id)
        else:
            self._loading = False
