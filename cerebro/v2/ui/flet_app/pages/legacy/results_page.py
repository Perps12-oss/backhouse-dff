"""Legacy results surface (unmounted). Workspace review is the live duplicate-results route."""
...
from cerebro.v2.ui.flet_app.components.files.results_list_card import build_results_list_group_card
from cerebro.v2.ui.flet_app.components.filters.filter_bar import FILTER_TABS, FILTER_TAB_ACCENTS, FilterBar
...
_FILTER_ACCENT = FILTER_TAB_ACCENTS
...
class ResultsPage(ft.Stack):
    ...
    def __init__(self, bridge: "StateBridge"):
        ...
        self._build_ui()
        self._chunked = ChunkedViewBuilder(self._bridge.flet_page)
        self._thumb_loader = ThumbnailSlotLoader(safe_update=ResultsPage._safe_update)
    ...
    def _build_ui(self) -> None:
        ...
        self._smart_row = SmartSelectionRow(
            t,
            variant="results",
            on_rule_change=self._on_smart_seg_change,
            on_apply=self._apply_smart_select,
            on_clear=self._unmark_all,
            visible=False,
        )
        self._smart_seg = self._smart_row.smart_seg
        self._rule_label = self._smart_row.rule_label
        ...
        self._filter_bar = FilterBar(
            t,
            self._on_filter_key_changed,
            show_files_suffix=True,
            dim_zero_counts=True,
            embedded=True,
        )
        self._filter_seg = self._filter_bar.segmented_button
        self._min_group_filter = ft.Dropdown(
            width=220,
            value="0",
            dense=True,
        ...
        self._inspector = FileInspector(self._bridge.flet_page, t)

        self.controls = [self._scroll_col, self._sticky_overlay, self._inspector, self._rendering_badge]
    ...
    # Filtering
    # ------------------------------------------------------------------
    def _on_filter_key_changed(self, key: str) -> None:
    ...
    def _render_grid_view(self, sc: list, filtered: list) -> None:
        ...
        cfg = RESULTS_GRID_CHUNK
        display_filtered, _grid_overflow = self._chunked.cap_items(filtered, RESULTS_GRID_CHUNK_CONFIG)
        if len(display_filtered) > cfg.async_threshold:
        ...
        self._chunked.render(
            ...
            filtered,
            config=cfg,
            card_builder=self._build_group_grid_section,
            trailing_builder=self._build_overflow_banner,
            on_complete=on_complete,
            on_abort=lambda: self._set_rendering(False),
        )
    ...
    def _render_list_view(self, sc: list, filtered: list) -> None:
        ...
        cfg = RESULTS_LIST_CHUNK
        if len(filtered) > cfg.async_threshold:
            self._set_rendering(True)
        ...
        self._chunked.render(
            ...
            self._groups,
            config=cfg,
            card_builder=lambda g, _i: self._build_or_get_group_card(g),
            trailing_builder=self._build_overflow_banner,
            after_chunk=after_chunk,
            on_complete=on_complete,
            on_abort=lambda: self._set_rendering(False),
            force_sync=len(filtered) <= cfg.async_threshold,
        )
        ...
        return ft.Container(
            content=ft.Row(
                [
                    ...
                    ft.Text(
                        f"{overflow:,} more groups not displayed — showing top {MAX_RENDERED_GROUPS:,} by reclaimable size.",
                        size=t.typography.size_sm,
                        color=t.colors.fg_muted,
                    ),
                ],
    ...
    def _build_group_card(self, group: DuplicateGroup, extra_badge: str | None = None) -> ft.Container:
        return build_results_list_group_card(
            self._t,
            group,
            icon_for_extension=self._file_type_icon,
    ...
    def _build_file_tile(self, f) -> ft.Container:
        return build_results_file_tile(
            self._t,
            f,
            icon_for_extension=self._file_type_icon,
            thumb_slots=self._thumb_slots,
            tile_cache=self._tile_cache_grid,
            on_open=self._open_inspector,
        )
    ...
    async def _load_grid_thumbnails_async(self, slots: Dict[str, ft.Container]) -> None:
        await self._thumb_loader.load_into_slots(
            slot_for_path=lambda path: slots.get(str(path)),
            paths=[Path(k) for k in slots.keys()],
            image_for_b64=lambda b64: ft.Image(
            ...
            batch_size=20,
            hosts=(self._results_grid,),
            log_slow=self._log_if_slow,
            slow_label="results:thumbnail_batch_apply",
            slow_ms=_UI_SLOW_MS,
        )
    ...
    def apply_theme(self, mode: str) -> None:
        ...
        self._inspector.sync_theme(self._t)
